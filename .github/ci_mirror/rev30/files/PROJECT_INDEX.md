...
**Current version:** 0.1.0-alpha.20-rev30  
...
- Dropbox current snapshot target: `EMMana-Next-0.1.0-alpha.20-rev29-source-artifact.zip`
...
## Rev28 — compare / tune workflow

- Geometry Undo/Redo: up to 30 steps, including endpoint drag, numeric edits, wire add/delete, feed assignment, template replacement and optimizer apply.
- Three latest saved variants are presented chronologically as A/B/C.
- Saved snapshots retain compact S11 Sweep traces and an exact local EMNX copy.
- Current Sweep can overlay A/B/C S11 traces with per-variant toggles.
- B/C relative-to-A summary shows dimensional and RF deltas.
- Open A/B/C restores exact geometry and intentionally invalidates old results until recalculation.
- Solver physics, schemas and optimization semantics remain unchanged.


## Rev29 — simple optimizer goals

- Main optimizer path is goal-first: Balance / Best matching / Maximum gain.
- User selects the frequency range directly; low-level algorithm controls remain under Technical details.
- Presets only map to the existing optimizer weights/limits; no optimizer backend or solver semantics changed.
- Advanced manual tuning remains available and is visibly marked as custom.


## Rev30 — unified parameterized antenna templates
- One registry-driven parameter UI for Dipole, GP, Square Loop, Moxon and 3/5/7-element Yagi.
- Parameters synthesize ordinary EMNX; no parallel model format was introduced.
- 5/7-element Yagi and Moxon are explicitly experimental until independent reference validation.