...
function updateMeta(p){els.name.textContent=p.name||'Без имени';const lossy=(p.wires||[]).filter(w=>Number(w.conductivity_s_per_m||0)>0).length;els.meta.textContent=`${fmt((p.frequency_hz||0)/1e6,3)} MHz · элементов ${p.wires?.length||0} · питание ${p.feeds?.length||0} · нагрузок ${p.loads?.length||0} · потерь в проводах ${lossy} · переменных ${p.design_variables?.length||0}`;renderVariables(p);}
...
function syncGeometryInspector(p){const w=ensureGeometrySelection(p),inputs=[els.geometrySx,els.geometrySy,els.geometrySz,els.geometryEx,els.geometryEy,els.geometryEz,els.geometryDiameter,els.geometrySegments];inputs.forEach(x=>{if(x)x.disabled=!w});els.geometryDelete.disabled=!w;els.geometryFeed.disabled=!w;if(!w){els.geometryWireId.textContent='—';els.geometryFeedStatus.textContent='Нет элементов антенны.';els.geometryFeedStatus.classList.remove('active');return}els.geometryWireId.textContent=w.id;[els.geometrySx,els.geometrySy,els.geometrySz].forEach((x,i)=>x.value=Number(w.start_m[i]).toFixed(4));[els.geometryEx,els.geometryEy,els.geometryEz].forEach((x,i)=>x.value=Number(w.end_m[i]).toFixed(4));els.geometryDiameter.value=(2000*Number(w.radius_m)).toFixed(3);els.geometrySegments.value=Number(w.segments);const feed=(p.feeds||[]).find(f=>f.wire_id===w.id);els.geometryFeedStatus.textContent=feed?'● Feed · segment '+feed.segment:'Питание не назначено этому элементу.';els.geometryFeedStatus.classList.toggle('active',!!feed)}
function antennaElementLabel(w){
  const id=String(w?.id||'');
  if(id==='REF')return 'Рефлектор';
  if(id==='DRV')return 'Активный элемент';
  const dm=id.match(/^DIR(\d+)$/);if(dm)return 'Директор '+dm[1];
  if(id==='MONO')return 'Излучатель';
  if(id==='W1')return 'Элемент 1';
  return id||'Элемент';
}
function projectTemplateId(p){
  const ids=new Set((p.wires||[]).map(w=>w.id));
  if(ids.has('REF')&&ids.has('DRV')&&ids.has('DIR1')){const n=[...ids].filter(x=>/^DIR\d+$/.test(x)).length+2;return n>=7?'yagi-7el':n>=5?'yagi-5el':'yagi-3el'}
  if(ids.has('DRV')&&ids.has('REF')&&ids.has('DTOP')&&ids.has('DBOT')&&ids.has('RTOP')&&ids.has('RBOT'))return 'moxon';
  if(ids.has('MONO'))return 'quarterwave-monopole-pec';
  if(ids.has('W1')&&ids.has('W2')&&ids.has('W3')&&ids.has('W4'))return 'square-loop';
  if((p.wires||[]).length===1&&ids.has('W1'))return 'halfwave-dipole';
  return null;
}
function isYagiProject(p){return /^yagi-/.test(projectTemplateId(p)||'')}
function drawYagiGuide(ctx,p,view){
  if(!isYagiProject(p)||geometryProjection!=='xz')return;
  const wires=(p.wires||[]).filter(w=>w.id==='REF'||w.id==='DRV'||/^DIR\d+$/.test(w.id));
  if(wires.length<3)return;
  const centers=wires.map(w=>({w,x:(Number(w.start_m[0])+Number(w.end_m[0]))/2,z:(Number(w.start_m[2])+Number(w.end_m[2]))/2})).sort((a,b)=>a.x-b.x);
  const first=view.toScreen([centers[0].x,0,0]),last=view.toScreen([centers[centers.length-1].x,0,0]);
  ctx.save();ctx.strokeStyle='#4b5565';ctx.lineWidth=3;ctx.setLineDash([7,6]);ctx.beginPath();ctx.moveTo(first.x,first.y);ctx.lineTo(last.x,last.y);ctx.stroke();ctx.setLineDash([]);ctx.fillStyle='#7f8a9a';ctx.font='11px Inter,system-ui';ctx.fillText('бум · визуальная ось',Math.min(first.x,last.x)+8,first.y-10);
  for(let i=0;i<centers.length-1;i++){const a=centers[i],b=centers[i+1],pa=view.toScreen([a.x,0,0]),pb=view.toScreen([b.x,0,0]),y=Math.max(pa.y,pb.y)+24;ctx.strokeStyle='#596579';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(pa.x,y);ctx.lineTo(pb.x,y);ctx.stroke();ctx.beginPath();ctx.moveTo(pa.x,y-4);ctx.lineTo(pa.x,y+4);ctx.moveTo(pb.x,y-4);ctx.lineTo(pb.x,y+4);ctx.stroke();ctx.fillStyle='#9aa7ba';ctx.textAlign='center';ctx.fillText(((b.x-a.x)*100).toFixed(1)+' см',(pa.x+pb.x)/2,y+15)}
  ctx.restore();
}
function renderGeometry(p){if(!els.geometryCanvas)return;const c=els.geometryCanvas,ctx=c.getContext('2d'),W=c.width,H=c.height;ctx.clearRect(0,0,W,H);ctx.fillStyle='#0b0d11';ctx.fillRect(0,0,W,H);const wires=p.wires||[],view=geometryTransform(p,geometryDrag?.view||null);geometryView=view;ctx.strokeStyle='#202632';ctx.lineWidth=1;for(let i=0;i<=10;i++){const x=40+(W-80)*i/10;ctx.beginPath();ctx.moveTo(x,28);ctx.lineTo(x,H-36);ctx.stroke()}for(let i=0;i<=7;i++){const y=28+(H-64)*i/7;ctx.beginPath();ctx.moveTo(40,y);ctx.lineTo(W-40,y);ctx.stroke()}const axes=geometryAxes[geometryProjection];ctx.fillStyle='#697384';ctx.font='12px Inter,system-ui';ctx.fillText(axes[2]+' →',W-55,H-12);ctx.fillText(axes[3]+' ↑',12,20);drawYagiGuide(ctx,p,view);for(const w of wires){const a=view.toScreen(w.start_m),b=view.toScreen(w.end_m),sel=w.id===selectedWireId,role=antennaElementLabel(w);ctx.strokeStyle=w.id==='DRV'?'#43d17c':sel?'#5ba0ff':'#9aa7ba';ctx.lineWidth=sel?6:4;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();const mx=(a.x+b.x)/2,my=(a.y+b.y)/2,len=Math.hypot(Number(w.end_m[0])-Number(w.start_m[0]),Number(w.end_m[1])-Number(w.start_m[1]),Number(w.end_m[2])-Number(w.start_m[2]));ctx.fillStyle=sel?'#d9e8ff':'#b4bdca';ctx.font='11px Inter,system-ui';ctx.textAlign='left';ctx.fillText(role,mx+8,my-8);if(isYagiProject(p)&&geometryProjection==='xz'){ctx.fillStyle='#78879a';ctx.font='10px Inter,system-ui';ctx.fillText((len*100).toFixed(1)+' см',mx+8,my+7)}if(sel){for(const pt of[a,b]){ctx.beginPath();ctx.fillStyle='#f4f6fb';ctx.strokeStyle='#3b82f6';ctx.lineWidth=3;ctx.arc(pt.x,pt.y,7,0,Math.PI*2);ctx.fill();ctx.stroke()}}const feed=(p.feeds||[]).find(f=>f.wire_id===w.id);if(feed){const t=Math.max(0,Math.min(1,(Number(feed.segment)+.5)/Math.max(1,Number(w.segments)))),x=a.x+(b.x-a.x)*t,y=a.y+(b.y-a.y)*t;ctx.beginPath();ctx.fillStyle='#43d17c';ctx.strokeStyle='#0b0d11';ctx.lineWidth=3;ctx.arc(x,y,8,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle='#7ee7a7';ctx.font='10px Inter,system-ui';ctx.fillText('Питание',x+10,y+4)}}syncGeometryInspector(p);const tid=projectTemplateId(p),title=tid?.startsWith('yagi-')?`Яги · ${wires.length} элемента`:tid==='halfwave-dipole'?'Диполь':tid==='quarterwave-monopole-pec'?'GP 1/4 λ':tid==='square-loop'?'Рамка':tid==='moxon'?'Moxon':'Антенна';els.geometrySummary.textContent=`${title} · ${geometryProjection.toUpperCase()} · ${wires.length} элементов · перемещайте элементы и пересчитывайте`}
...
function addGeometryWire(){try{const p=project(),wires=p.wires||(p.wires=[]),used=new Set(wires.map(w=>w.id));let n=1;while(used.has('W'+n))n++;const id='W'+n,[a,b]=geometryAxes[geometryProjection],freq=Number(p.frequency_hz)||299792458,lambda=299792458/freq,len=.5*lambda,gap=.18*lambda,bounds=geometryBounds(p),centerA=wires.length?bounds.maxA+gap:0,centerB=(bounds.minB+bounds.maxB)/2,hidden=[0,1,2].find(x=>x!==a&&x!==b),start=[0,0,0],end=[0,0,0];start[a]=end[a]=centerA;start[b]=centerB-len/2;end[b]=centerB+len/2;start[hidden]=end[hidden]=0;wires.push({id,start_m:start,end_m:end,radius_m:.001,segments:21});selectedWireId=id;geometryCommit(p,'элемент добавлен')}catch(e){status('ошибка добавления','err');els.diag.textContent=String(e.message||e)}}
function deleteGeometryWire(){try{const p=project(),w=geometryWire(p);if(!w)return;if((p.wires||[]).length<=1)throw new Error('Нельзя удалить последний элемент антенны');const referenced=(p.design_variables||[]).some(v=>v.wire_id===w.id)||(p.design_constraints||[]).some(c=>c.wire_a===w.id||c.wire_b===w.id);if(referenced)throw new Error('Элемент используется в переменных или ограничениях оптимизации. Сначала измените их в EMNX.');p.wires=p.wires.filter(x=>x.id!==w.id);p.loads=(p.loads||[]).filter(x=>x.wire_id!==w.id);p.feeds=(p.feeds||[]).filter(x=>x.wire_id!==w.id);if(!p.feeds.length&&p.wires.length){const q=p.wires[0];p.feeds=[{wire_id:q.id,segment:Math.floor((Number(q.segments)-1)/2),voltage_re_v:1,voltage_im_v:0}]}selectedWireId=p.wires[0]?.id||null;geometryCommit(p,'элемент удалён')}catch(e){status('удаление отменено','err');els.diag.textContent=String(e.message||e)}}
function assignGeometryFeed(){try{const p=project(),w=geometryWire(p);if(!w)throw new Error('Выберите элемент антенны');p.feeds=[{wire_id:w.id,segment:Math.floor((Number(w.segments)-1)/2),voltage_re_v:1,voltage_im_v:0}];geometryCommit(p,'питание назначено')}catch(e){status('ошибка назначения питания','err');els.diag.textContent=String(e.message||e)}}
...
async function loadGeometryTemplates(){if(!els.geometryTemplate)return;try{let r;try{r=await api('/api/templates')}catch(apiError){const q=await fetch('/template-registry.json',{cache:'no-store'});if(!q.ok)throw apiError;r=await q.json()}geometryTemplates=(r.templates||[]).filter(x=>x.status==='implemented');els.geometryTemplate.innerHTML='';for(const item of geometryTemplates){const o=document.createElement('option');o.value=item.id;o.textContent=item.label;els.geometryTemplate.appendChild(o)}const p=project(),detected=projectTemplateId(p),preferred=geometryTemplates.find(x=>x.id===detected)||geometryTemplates[0];if(preferred){els.geometryTemplate.value=preferred.id;activeTemplateId=detected===preferred.id?preferred.id:null}syncGeometryTemplateStatus();renderTemplateParameterPanel();if(activeTemplateId)syncTemplateParameterPanelFromProject(p);if(p.frequency_hz)els.geometryTemplateFrequency.value=(Number(p.frequency_hz)/1e6).toFixed(3)}catch(e){geometryTemplates=[];activeTemplateId=null;els.geometryTemplate.innerHTML='<option value="">Шаблоны недоступны</option>';els.geometryTemplateStatus.textContent='Каталог шаблонов не загрузился: '+String(e.message||e);els.geometryTemplateStatus.classList.remove('ready');if(els.templateParamPanel)els.templateParamPanel.hidden=true;if(els.yagiQuickPanel)els.yagiQuickPanel.hidden=true}}
