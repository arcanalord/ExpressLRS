...
def load_template_registry():
    candidates=[TEMPLATE_REGISTRY, STATIC / "template-registry.json"]
    errors=[]
    ...
    raise FileNotFoundError("template registry unavailable: " + " | ".join(errors))
...
def synthesize_template(template_id: str, target_frequency_hz: float, raw_parameters: dict|None=None) -> dict:
    ...
    registry=load_template_registry(); item=next((x for x in registry.get("templates",[]) if x.get("id")==template_id),None)
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/api/templates":
            try: self.send_json(load_template_registry())
            except Exception as e: self.send_json({"error": str(e)}, HTTPStatus.INTERNAL_SERVER_ERROR)
            return
