...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='emnext-ui-cache-') as td:
        ...
        try:
            html,ctype=get(base+'/');text=html.decode('utf-8');assert ctype=='text/html' and 'help-trigger' in text and 'ground-mode' in text and 'mobile-menu' in text and 'measurement-file' in text and 'measurement-compare' in text and 'measurement-chart' in text and 'variant-save' in text and 'measurement-bind' in text and 'report-generate' in text and 'report-preview' in text and 'geometry-canvas' in text and 'geometry-add' in text and 'geometry-feed' in text and 'geometry-delete' in text and 'geometry-template' in text and 'geometry-template-create' in text and 'data-view-tab="geometry"' in text
            ...
            loop=get_json(base+'/api/template/square-loop?frequency_hz=300000000');assert loop['template_id']=='square-loop' and len(loop['project']['wires'])==4
            loopcheck=post_json(base+'/api/model-check',{'project':loop['project'],'kernel':'reduced'},30);assert loopcheck['ok'] and 'MODEL OK' in loopcheck['stdout']
            ...
            appjs,_=get(base+'/app.js');apptext=appjs.decode('utf-8');assert 'importMeasurement' in apptext and 'compareMeasurement' in apptext and 'drawMeasurementComparison' in apptext and 'escapeHtml' in apptext and 'saveVariant' in apptext and 'bindMeasurement' in apptext and 'generateReport' in apptext and 'downloadReport' in apptext and 'renderGeometry' in apptext and 'addGeometryWire' in apptext and 'deleteGeometryWire' in apptext and 'assignGeometryFeed' in apptext and 'geometryPointerMove' in apptext and 'loadGeometryTemplates' in apptext and 'createGeometryTemplate' in apptext
