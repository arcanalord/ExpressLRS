...
ResultSet solve_wire_mom_experimental(const Project&project,const WireMomSettings&settings){
    ...
    result.warnings=check.warnings;if(!(residual<1e-10))result.warnings.emplace_back("Linear-system normalized residual exceeds 1e-10");result.warnings.insert(result.warnings.end(),kernel_warnings.begin(),kernel_warnings.end());if(settings.kernel_mode==WireKernelMode::Reduced)result.warnings.emplace_back("Experimental Wire-MoM uses a reduced-radius kernel; NEC/full-wave gate is still required");if(project.ground.model=="free-space")result.warnings.emplace_back("Environment is free space");else if(project.ground.model=="pec-image-v1")result.warnings.emplace_back("Alpha.20 PEC ground image boundary active; lower half-space pattern is suppressed");else{result.warnings.emplace_back("Alpha.20 finite ground uses complex image/reflection approximation; Sommerfeld/Norton is not yet implemented");result.warnings.emplace_back("Alpha.20 ground dissipation is inferred by accepted-power closure, not a direct volumetric loss integral");}return result;
}
}
