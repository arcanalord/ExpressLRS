...
async function loadReferenceStatus(){try{const r=await api('/api/reference-status');const ref=r.independent_reference||{},internal=r.internal_numerical_gates||{};const internalOk=Object.values(internal).every(x=>String(x).toLowerCase()==='pass');const necPass=String(ref.ci_status||'').toLowerCase()==='pass'||String(ref.local_status||'').toLowerCase()==='pass';const el=$('#reference-status');el.textContent=necPass?'NEC2 PASS':(internalOk?'INTERNAL PASS · NEC2 NOT RUN':'TRUST CHECK');el.className='status '+(necPass?'ok':(internalOk?'':'err'));el.title=`NEC2 local: ${ref.local_status||'unknown'} · CI: ${ref.ci_status||'unknown'}${ref.local_reason?' · '+ref.local_reason:''}`}catch(e){const el=$('#reference-status');el.textContent='TRUST UNKNOWN';el.className='status err'}}
function project(){return JSON.parse(els.editor.value)}
function updateMeta(p){els.name.textContent=p.name||'Без имени';const lossy=(p.wires||[]).filter(w=>Number(w.conductivity_s_per_m||0)>0).length;els.meta.textContent=`${fmt((p.frequency_hz||0)/1e6,3)} MHz · ${p.wires?.length||0} wires · ${p.feeds?.length||0} feed · ${p.loads?.length||0} loads · ${lossy} lossy wires · ${p.design_variables?.length||0} vars`;renderVariables(p);}
async function loadExample(){const name=els.example.value;if(!name)return;const p=await api('/api/example/'+encodeURIComponent(name));els.editor.value=JSON.stringify(p,null,2);updateMeta(p);const mhz=(p.frequency_hz||0)/1e6;if(mhz>0){els.sweepStart.value=(mhz*.9).toFixed(3);els.sweepStop.value=(mhz*1.1).toFixed(3);els.optBandStart.value=(mhz*.99).toFixed(3);els.optBandStop.value=(mhz*1.01).toFixed(3)}status('модель загружена','ok')}
...
function drawCurrents(result){const c=els.currents,ctx=c.getContext('2d'),W=c.width,H=c.height;ctx.clearRect(0,0,W,H);ctx.fillStyle='#0b0d11';ctx.fillRect(0,0,W,H);const vals=(result.segment_currents_a||[]).map(z=>Math.hypot(z.re,z.im));if(!vals.length)return;const m=Math.max(...vals,1e-12),pad=35,bw=(W-2*pad)/vals.length;ctx.strokeStyle='#202735';for(let k=0;k<=4;k++){const y=pad+(H-2*pad)*k/4;ctx.beginPath();ctx.moveTo(pad,y);ctx.lineTo(W-pad,y);ctx.stroke()}ctx.fillStyle='#5ba0ff';vals.forEach((v,i)=>{const h=(H-2*pad)*v/m;ctx.fillRect(pad+i*bw,H-pad-h,Math.max(1,bw*.72),h)});ctx.fillStyle='#798499';ctx.font='12px system-ui';ctx.fillText(`max |I| = ${m.toExponential(3)} A`,pad,20);ctx.fillText(`${vals.length} segments`,W-130,H-12)}
function s11db(s){const m=Math.hypot(Number(s?.re||0),Number(s?.im||0));return m>0?20*Math.log10(m):-300}
function drawSweep(sw){const c=els.sweepChart,ctx=c.getContext('2d'),W=c.width,H=c.height;ctx.clearRect(0,0,W,H);ctx.fillStyle='#0b0d11';ctx.fillRect(0,0,W,H);const pts=sw.samples||[];if(pts.length<2)return;const padL=64,padR=64,padT=30,padB=42,x0=pts[0].frequency_hz,x1=pts[pts.length-1].frequency_hz;const dbs=pts.map(p=>s11db(p.s11));const vs=pts.map(p=>Math.min(Number(p.vswr||1),10));let yDbMin=Math.min(-10,...dbs),yDbMax=0;if(yDbMin<-60)yDbMin=-60;const X=f=>padL+(W-padL-padR)*(f-x0)/(x1-x0||1),Ydb=d=>padT+(H-padT-padB)*(yDbMax-d)/(yDbMax-yDbMin||1),Yv=v=>padT+(H-padT-padB)*(v-1)/9;ctx.strokeStyle='#202735';ctx.lineWidth=1;for(let k=0;k<=5;k++){const y=padT+(H-padT-padB)*k/5;ctx.beginPath();ctx.moveTo(padL,y);ctx.lineTo(W-padR,y);ctx.stroke()}for(let k=0;k<=6;k++){const x=padL+(W-padL-padR)*k/6;ctx.beginPath();ctx.moveTo(x,padT);ctx.lineTo(x,H-padB);ctx.stroke()}ctx.strokeStyle='#5ba0ff';ctx.lineWidth=3;ctx.beginPath();pts.forEach((p,i)=>{const x=X(p.frequency_hz),y=Ydb(dbs[i]);i?ctx.lineTo(x,y):ctx.moveTo(x,y)});ctx.stroke();ctx.strokeStyle='#43d17c';ctx.lineWidth=2;ctx.beginPath();pts.forEach((p,i)=>{const x=X(p.frequency_hz),y=Yv(vs[i]);i?ctx.lineTo(x,y):ctx.moveTo(x,y)});ctx.stroke();if(sw.resonance_frequency_hz){const x=X(sw.resonance_frequency_hz);ctx.strokeStyle='#ffbd59';ctx.setLineDash([6,5]);ctx.beginPath();ctx.moveTo(x,padT);ctx.lineTo(x,H-padB);ctx.stroke();ctx.setLineDash([])}ctx.fillStyle='#798499';ctx.font='12px system-ui';ctx.fillText(`${(x0/1e6).toFixed(1)} MHz`,padL,H-15);ctx.fillText(`${(x1/1e6).toFixed(1)} MHz`,W-padR-70,H-15);ctx.fillStyle='#5ba0ff';ctx.fillText('S11 dB',padL,18);ctx.fillStyle='#43d17c';ctx.fillText('VSWR 1…10',padL+75,18);ctx.fillStyle='#ffbd59';ctx.fillText('resonance marker',padL+180,18)}
function renderVariables(p){const vars=p.design_variables||[],cons=p.design_constraints||[];if(!vars.length){els.optVariables.classList.add('muted');els.optVariables.textContent='В проекте нет design_variables.'}else{els.optVariables.classList.remove('muted');const valueOf=v=>{const w=(p.wires||[]).find(x=>x.id===v.wire_id);if(!w)return NaN;const a=w.start_m,b=w.end_m;if(v.kind==='wire_center_x_m')return(a[0]+b[0])/2;if(v.kind==='wire_length_m')return Math.hypot(b[0]-a[0],b[1]-a[1],b[2]-a[2]);return NaN};els.optVariables.innerHTML=vars.map(v=>`<div class="opt-row"><span>${v.id}<small> ${v.kind}</small></span><b>${fmt(valueOf(v),4)} · ${fmt(v.min,4)}…${fmt(v.max,4)}</b></div>`).join('')}if(cons.length){els.optConstraints.classList.remove('muted');els.optConstraints.innerHTML='<div class="constraint-head">Constraints</div>'+cons.map(c=>`<div class="opt-row"><span>${c.id}<small> ${c.kind}</small></span><b>${fmt(c.value,4)}</b></div>`).join('')}else{els.optConstraints.classList.add('muted');els.optConstraints.textContent='Constraints не заданы.'}}
...
function candidateRows(c){const m=c.metrics||{};return `worst VSWR ${fmt(m.vswr,3)} · min Gain ${fmt(m.forward_gain_dbi,2)} dBi · min F/B ${fmt(m.front_to_back_db,2)} dB · Boom ${fmt(m.boom_length_m,3)} m`}
...
async function runSweep(){setWorkspaceView('solve');try{const p=project();updateMeta(p);const startHz=Number(els.sweepStart.value)*1e6,stopHz=Number(els.sweepStop.value)*1e6,points=Number(els.sweepPoints.value),parallel_workers=Number(els.parallelWorkers.value);if(!(parallel_workers>=1&&parallel_workers<=16))throw new Error('Workers: 1…16');if(!(startHz>0&&stopHz>=startHz&&points>=2))throw new Error('Проверь диапазон Sweep');els.runSweep.disabled=true;status('sweep…');const j=await api('/api/sweep',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({project:p,kernel,start_hz:startHz,stop_hz:stopHz,points,parallel_workers})});const sw=j.sweep;lastSweep=sw;drawSweep(sw);if(lastMeasurement)els.measurementCompare.disabled=false;els.sweepSummary.textContent=`res ${fmt(sw.resonance_frequency_hz/1e6,3)} MHz · min S11 ${fmt(sw.min_s11_db,2)} dB @ ${fmt(sw.min_s11_frequency_hz/1e6,3)} MHz · BW10 ${fmt(sw.bandwidth_10db_hz/1e6,3)} MHz · ${sw.execution_workers}w${j.cache_hit?' · cache':''}`;status(j.cache_hit?'sweep из кэша':'sweep завершён','ok')}catch(e){status('ошибка sweep','err');els.diag.textContent=String(e.message||e)}finally{els.runSweep.disabled=false}}
...
let initialView='solve';try{initialView=localStorage.getItem('emnext.workspaceView')||'solve'}catch{}setWorkspaceView(initialView);
...
const _updateMeta=updateMeta;updateMeta=function(p){_updateMeta(p);syncGroundFromProject(p);};
...
function setWorkspaceView(view){
  const valid=new Set(['model','solve','optimize','measurement']);
  if(!valid.has(view))view='solve';
  document.querySelectorAll('[data-view]').forEach(node=>{node.hidden=node.dataset.view!==view});
  viewTabs.forEach(btn=>btn.classList.toggle('active',btn.dataset.viewTab===view));
  try{localStorage.setItem('emnext.workspaceView',view)}catch{}
}
viewTabs.forEach(btn=>btn.addEventListener('click',()=>setWorkspaceView(btn.dataset.viewTab)));
