...
const navigationController=createNavigationController({
  initialView:'chats',
  onNetwork:()=>renderRoute(),
  onMap:()=>renderMap(),
});
function setView(name){return navigationController.setView(name);}
...
function currentScreenHelpTopic(){ return SCREEN_HELP_TOPICS[navigationController.current()] || HELP_FALLBACK_ID; }
...
async function shareCurrentPosition({preferBrowser=true}={}){
  ...
  browserMapPosition={...position};if(navigationController.current()==='map')renderMap({fallback:browserMapPosition});
...
function wireEvents() {
  ...
  globalThis.addEventListener('online',()=>{if(navigationController.current()==='map')renderMap({preserveViewport:true});});
  globalThis.addEventListener('offline',()=>{mapTileState='offline';if(navigationController.current()==='map')renderMap({preserveViewport:true});});
...
runAppBootstrap({
  initTheme,
  bindRadioState:()=>{
    mockRadio.onState(snapshot=>{if(activeRadio===mockRadio)renderRadioState(snapshot);});
    webSerialRadio.onState(snapshot=>{if(activeRadio===webSerialRadio)renderRadioState(snapshot);});
    androidBleRadio.onState(snapshot=>{if(activeRadio===androidBleRadio)renderRadioState(snapshot);});
  },
  renderInitial:()=>{renderConversations();renderConversationHeader();renderMessages();renderTransportCards();renderGlobalStatus();renderRoute();renderDeliveryQueue();renderSystemBanner('syncing');renderMap();},
  restoreEngineeringUi:()=>{const eng=storageGet('engineering')==='1';$('#engineeringToggle').checked=eng;$('#engineeringBlock').hidden=!eng;},
  wireNavigation:()=>navigationController.wire(),
  wireEvents,
  openInitialHelp:()=>{const initialHelpTopic=parseHelpTopic(location.search);if(initialHelpTopic)openFullHelp(initialHelpTopic,{pushHistory:false});},
  consumePendingDeepLink:()=>{if(globalThis.__meshPendingDeepLink){handleAndroidDeepLink(globalThis.__meshPendingDeepLink);globalThis.__meshPendingDeepLink='';}},
  autoConnect:()=>{if(!globalThis.__meshDisableAutoConnect)activateRadio(AndroidBridgeMeshtasticTransport.isSupported()?androidBleRadio:mockRadio).catch(()=>{});},
  installDebug:()=>{window.__meshDebug={transportManager,queue,mockRadio,androidBleRadio,getPhoneApiSnapshot:()=>currentSnapshot(),getCurrentConversation:()=>currentConversation(),selectConversation,sendMessage,retryQueueItem,renderDeliveryQueue,exportPhoneApiCapture,simulateReboot:()=>mockRadio.simulateReboot(),setNextRoutingResult:(code)=>mockRadio.setNextRoutingResult(code),setFileDisconnectAfter:(n)=>mockRadio.setFileDisconnectAfter(n),getConversationUi:(id)=>({...uiState(id)}),sendSmallFile,getFileTransfers:()=>fileTransfers.list().map(t=>({id:t.id,name:t.name,status:t.status,confirmed:fileTransfers.confirmedCount(t.id),total:t.total,targetNode:t.targetNode,conversationId:t.conversationId,error:t.error})),simulateFileSubsystemRestart,resumeIncompleteFileTransfers,renderMap,getMapViewport:()=>({...mapViewport,pointers:undefined,pinch:undefined}),getCurrentView:()=>navigationController.current(),locateOnMap,shareCurrentPosition,currentShareUrls,renderShareQr,trackHistory,getWaypoints:()=>[...sharedWaypoints.values()],sendWaypointFromModal,helpRegistry,renderQuickHelp,openFullHelp,copySafeDiagnostics,currentHelpTopic:()=>currentHelpTopicId};},
  registerServiceWorker:()=>{if('serviceWorker' in navigator&&location.protocol.startsWith('http')&&!AndroidBridgeMeshtasticTransport.isSupported())navigator.serviceWorker.register('./sw.js').catch(()=>{});},
});
